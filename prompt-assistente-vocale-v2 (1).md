...
- IA: Google Gemini 3.1 Flash Lite API (gratuita, free tier 15 RPM / 250K TPM / 500 RPD) per interpretare i comandi
...
### 2. Cervello unificato (Gemini 3.1 Flash Lite)
...
**Dettagli tecnici endpoint Gemini 3.1 Flash Lite:**
- URL: `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-lite-latest:generateContent?key=API_KEY`
- Per forzare risposta JSON pulita usa `generationConfig.responseMimeType: 'application/json'` + opzionalmente `responseSchema` con lo schema dell'oggetto atteso
- Per audio Telegram: invia il file audio come `inline_data` con `mime_type: 'audio/ogg'` (i vocali Telegram sono OGG/Opus)
- Limiti free tier: 15 richieste/minuto, 250K token/minuto, 500 richieste/giorno — più che sufficienti per uso personale
